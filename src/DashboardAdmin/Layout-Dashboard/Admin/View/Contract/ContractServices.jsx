...
const hendleNavgateToService=(serviceID)=>{
  ...
  return (
    ...
    <div className="container mx-auto p-4">
      <h1 className="text-2xl font-bold mb-4">عقود الخدمات</h1>
      <table className="min-w-full border border-gray-300">
        <thead>
          <tr>
            ...
            <th className="bg-gray-100 p-2 border-b">الخدمة</th>
            ...
            <th className="bg-gray-100 p-2 border-b"> عرض الخدمة</th>
        ...
        <tbody>
          {contracts.map((contract) => (
            <tr key={contract.id}>
              ...
              <td className="p-2 border-b">{contract.client.firstName}</td>
              ...
              <td className="p-2 border-b">{contract.service.title}</td>
              <td className="p-2 border-b">{moment(contract.start_date).format('dddd, D MMMM YYYY, h:mm A')}</td> 
              <td className="p-2 border-b">{moment(contract.end_date).format('dddd, D MMMM YYYY, h:mm A')}</td>
              <td className="p-2 border-b">{calculateRemainingTime(contract.start_date, contract.end_date)}</td>
              <td className="p-2 border-b">{contract.price}</td>
              <td className="p-2 border-b">
